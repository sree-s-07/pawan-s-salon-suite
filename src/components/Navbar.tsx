...
import { Menu, X, Phone, UserCog, User, LogOut, Sparkles } from "lucide-react";
            ...
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.3 }} className="lg:hidden glass mt-4 mx-4 rounded-2xl overflow-hidden">
              <div className="p-6 space-y-2">
                ...
                <div className="pt-4 border-t border-[#E8DDD8] space-y-2">
                  {loggedIn ? (
                    <button onClick={handleUserLogout} className="w-full px-4 py-3 rounded-xl text-base font-medium text-[#E07A5F] hover:bg-[#F5E6E0] text-left flex items-center gap-2">
                      <LogOut className="h-4 w-4" /> Logout
                    </button>
                  ) : (
                    <Link to="/login" onClick={() => setOpen(false)} className="block px-4 py-3 rounded-xl text-base font-medium text-[#2C1810] hover:bg-[#F5E6E0]">Sign In</Link>
                  )}
